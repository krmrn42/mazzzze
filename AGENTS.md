...
- **Monster system** (`REQ-0019`/`REQ-0020`): `Monster` (`src/Monster.cs`, abstract `CharacterBody3D`) is the template; `Wukong` (`src/Wukong.cs`) the first concrete type (wolf, contact). Static `Monster.All` registry (persistent under `Main`, not chunk-bound). Perception = vision cone + LoS ray (`CanSee`); FSM in `_PhysicsProcess` — `Cycle` (patrol) / `Threat` (chase) / `Stun` / `Distract`, priority Stun > player-visibility > distraction, no memory after disruption. Movement = **BFS over `MazeData.IsFloor` cells** (patrol restricted to a segment ≈ 1 chunk); model faces movement via `LookAt` + `ModelYawOffsetDeg` (90° — wolf nose is along model X). Animation (`UpdateAnim`/`PlayAttack`) plays the glb's `AnimationPlayer` clips: `MoveAnim` looped + speed-scaled, `AttackAnim` one-shot on contact — the wolf asset has **only combat clips (no walk/idle)**, so `atk01_loop` is reused for locomotion + `bite01` on contact. Contact damage emits `PlayerHit` signal + `DamageHud` red flash (no health system yet). Model scaled by **length** (low/long wolf), local-space AABB (float32 at world −18000). `MonsterSpawner` (`Main/MonsterSpawner`) is a **minimal debug spawner** near start (real spawner is future). Hooks/TODO: `Stun()` has no trigger (future ball IDEA-0025), distraction reacts to any `WorldItem`, Ranged/Small/health/animations are future.
- **Mob.cs** / **mob.tscn** — old charge stub, superseded by `Monster`/`Wukong`, never spawned.
